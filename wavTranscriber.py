...
def stt(ds, audio, fs):
    ...
    metadata = ds.sttWithMetadata(audio, num_results=3) # prob. better to only call this to avoid double infer.
    ...
    metadata_list = list()
    # print(metadata)
    for candidate_transcript in metadata.transcripts:
        candidate_list = list()
        for token_metadata in candidate_transcript.tokens:
            candidate_list.append((token_metadata.text, token_metadata.timestep, token_metadata.start_time))
        metadata_list.append(candidate_list)

    # return time_start, time_end, ' '.join(transcript.split()), metadata_list
    return [transcript, metadata_list, inference_time]
